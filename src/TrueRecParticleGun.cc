...
TrueRecParticleGun::TrueRecParticleGun()
{
    ...
    photon_dphi_v_2D_TAPS   = new TH3F("photon_dphi_v_2D_TAPS", "#gamma TAPS; E_{rec} (MeV); #theta_{rec} (^{o});  #phi_{rec} - #phi_{true} (^{o})", 75, 0., 1500., 30, 0, 30, 160, -20., 20.);
    proton_dphi_v_2D_TAPS   = new TH3F("proton_dphi_v_2D_TAPS", "proton; E_{rec} (MeV); #theta_{rec} (^{o}); #phi_{rec} - #phi_{true} (^{o})", 150, 0., 1500., 60, 0, 30, 160, -20., 20.);

    proton_EvDE_TAPS        = new TH2F("proton_EvDE_TAPS", "proton; E (MeV); #Delta E (MeV)", 500, 0., 500., 100, 0., 10.);
...
//    TrueObs.Start(*GetGeant());
    ...
    if( particle == 1 )
    {
        ...
        {
            Double_t E_rec    =  GetTracks()->GetClusterEnergy(i);
            Double_t th_rec    =  GetTracks()->GetClusterEnergy(i);
